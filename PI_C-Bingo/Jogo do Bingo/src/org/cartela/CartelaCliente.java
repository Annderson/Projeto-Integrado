...
/**Esta interface CartelaCliente é a cartela do jogador propriamente dita com 24 números válidos,
 * *também se encontra 3 (três) botões bingo que se for pressionado dará respectivas
 * mensagens de vencedor ou perdedor, o botão sair, o jogador pode abandonar o jogo
 * em andamento, quando bem entender e. O botão voltar que retorna ao para classe TelaInicial, 
				...
				while(true){
					if (vf == 0){
						...
						if (jogador.pedido()){
						    ...
						    if(recepcao.equals("Você perdeu, ja ouve um ganhador"))
			...
			public void actionPerformed(ActionEvent e){
				try {
					if (verificarBingo()){
						...
						lblNumeroSorteado.setText("Bingooo Parabéns " + nome);
					}
					else
						JOptionPane.showMessageDialog(null, "Xiii " + nome + " você não fez bingo, otario!");
				} catch (Exception e2) {
					 e2.printStackTrace();
				}
